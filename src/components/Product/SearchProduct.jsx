...
function SearchProduct(props) {
    ...
    let { page, slug, search } = useParams();
    page = parseInt(page)
    const [count, setCount] = useState(1)
    const [products, setProducts] = useState([])
    const [categories, setCategories] = useState([]);
    const href = '/products/' + slug + '/' + search + '/'
    useEffect(() => {
        const fetchProducts = async () => {
            let listPr = []
            if (slug === 'all') {
                listPr = await productAPI.searchProduct(search, 'all', page);
            } else {
                listPr = await productAPI.searchProduct(search, slug, page);
            }
    ...
    return (
        <div>
            <SearchForm Search = {search} Slug={slug} Categories={categories} HandleCateChange={handleCateChange} />
